...
export function Nav() {
  return (
    <nav className="navbar">
      ...
      <Link className="nav-item" to="/books-list">
        Books
      </Link>
      <Link className="nav-item" to="/shopping-list">
        Groceries
      </Link>
    </nav>
  );
...
export function NavMUI() {
  ...
  function mapToItemType(value: string): ItemType {
    switch (value) {
      case "/watchlist":
        return ItemType.WATCHLIST;
      case "/books-list":
        return ItemType.BOOKS_LIST;
      case "/shopping-list":
        return ItemType.SHOPPING_LIST;
      case "/all":
        return ItemType.ALL;
      default:
        return ItemType.ALL;
    ...
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Tab
          ...
          icon={<WatchLater />}
          value="/watchlist"
          label={intl.formatMessage({ id: "nav.movies" })}
        />
        <Tab
          className={classes.tabColor}
          icon={<MenuBook />}
          value="/books-list"
          label={intl.formatMessage({ id: "nav.books" })}
        />
        <Tab
          className={classes.tabColor}
          icon={<ShoppingCart />}
          value="/shopping-list"
          label={intl.formatMessage({ id: "nav.groceries" })}
        />
        <Tab
          className={classes.tabColor}
          value="/all"
          label={intl.formatMessage({ id: "nav.all" })}
        />
      </Tabs>
    </Box>
  );
